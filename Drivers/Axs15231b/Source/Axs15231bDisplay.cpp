...
void Axs15231bDisplay::lvgl_port_flush_callback(lv_display_t *drv, const lv_area_t *area, uint8_t *color_map) {
    ...
    if (rotation != LV_DISPLAY_ROTATION_0) {
        ...
        switch (rotation) {
                ...
                for (int y = 0; y < logical_height; y++) {
                    for (int x = 0; x < logical_width; x++) {
                        int new_x = y;
                        int new_y = logical_width - 1 - x;
                        dst[new_y * self->configuration->horizontalResolution + new_x] = src[y * logical_width + x];
                        }
                    }
                    break;
                ...
                for (int y = 0; y < logical_height; y++) {
                    for (int x = 0; x < logical_width; x++) {
                        int new_x = logical_width - 1 - x;
                        int new_y = logical_height - 1 - y;
                        dst[new_y * self->configuration->horizontalResolution + new_x] = src[y * logical_width + x];
                        }
                    }
                    break;
        ...
        draw_buf = self->temp_buf;
}

    esp_lcd_panel_draw_bitmap(panel_handle, 0, 0, self->configuration->horizontalResolution, self->configuration->verticalResolution, draw_buf);
...
bool Axs15231bDisplay::createPanelHandle() {
    ...
    //SWAPXY Doesn't work with the JC3248W535
...
Axs15231bDisplay::Axs15231bDisplay(std::unique_ptr<Configuration> inConfiguration) :
    ...
    temp_buf = (uint16_t *)heap_caps_malloc(configuration->horizontalResolution * configuration->verticalResolution * sizeof(uint16_t), MALLOC_CAP_SPIRAM);
}
...
bool Axs15231bDisplay::stop() {
    ...
    if (ioHandle != nullptr && esp_lcd_panel_io_del(ioHandle) != ESP_OK) {
        TT_LOG_E(TAG, "Failed to delete IO");
    }
    ioHandle = nullptr;
...
bool Axs15231bDisplay::startLvgl() {
    ...
    if (buffer1 == nullptr) {// || buffer2 == nullptr
        TT_LOG_E(TAG, "Failed to allocate buffers");
        return false;
    }

    lv_display_set_buffers(lvglDisplay, buffer1, nullptr, bufferSize, LV_DISPLAY_RENDER_MODE_FULL);
