...
constexpr auto* SETTINGS_FILENAME = "settings.properties";
...
// Accept legacy / typo variant for backward compatibility
constexpr auto* KEY_AP_PASSWORD_LEGACY = "apPasswrod"; // intentional misspelling
...
WebServerSettings loadOrGetDefault() {
    ...
    if (!loadedFromFlash) {
        // First boot - use defaults (WiFi ON, WebServer OFF)
        settings = getDefault();
        // Save defaults to flash so toggle states persist
        save(settings);
        TT_LOG_I(TAG, "First boot - saved default settings (WiFi ON, WebServer OFF)");
    }
