...
class WebServerSettingsApp final : public App {
    ...
    static void onWifiModeChanged(lv_event_t* e) {
        ...
        getMainDispatcher().dispatch([app, dropdown, index] {
    ...
    static void onSyncAssets(lv_event_t* e) {
        auto* app = static_cast<WebServerSettingsApp*>(lv_event_get_user_data(e));
        TT_LOG_I("WebServerSettings", "Manual asset sync triggered");
        
        // Run sync on main dispatcher (may briefly block UI during file I/O)
        getMainDispatcher().dispatch([]{ 
            bool success = service::webserver::syncAssets();
            if (success) {
                TT_LOG_I("WebServerSettings", "Asset sync completed successfully");
            } else {
                TT_LOG_E("WebServerSettings", "Asset sync failed");
            }
        });
    }
...
#else
        lv_label_set_text(labelUrlValue, "Simulator N/A");
#endif
    }
    ...
    void onHide(TT_UNUSED AppContext& app) override {
        if (updated) {
            ...
            getMainDispatcher().dispatch([copy, wifiChanged, webServerChanged]{
                ...
                if (wifiChanged) {
                    TT_LOG_I("WebServerSettings", "WiFi mode changed to %s",
                             copy.wifiMode == settings::webserver::WiFiMode::AccessPoint ? "AP" : "Station");
                }

                // Control WebServer service immediately
                if (webServerChanged) {
                    TT_LOG_I("WebServerSettings", "WebServer %s",
                             copy.webServerEnabled ? "enabling..." : "disabling...");
                    service::webserver::setWebServerEnabled(copy.webServerEnabled);
                }
            });
        }

        // Publish event when hiding if no changes were saved (otherwise already published in dispatch)
        if (!updated) {
            kernel::publishSystemEvent(kernel::SystemEvent::WebServerSettingsChanged);
        }
    }
