...
struct WebServerSettings {
    ...
    std::string apSsid;           // Default: "TDeck-XXXX" (last 4 of MAC)
 ...
 * @return SSID string in format "TDeck-XXXX"
 */
std::string generateDefaultApSsid();

/**
 * NOTE: Flash-persistent settings are now handled automatically via SettingsSync
 * - Boot-time sync: /sdcard/.tactility/settings/webserver.properties → /data/settings/webserver.properties
 * - Runtime sync: save() automatically backs up to SD card
 * - No manual credential save/load needed!
 */
